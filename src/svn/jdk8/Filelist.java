	...
	static PreparedStatement ps=null;
	   public static void filelist(String svnurl2,String username2,String password2,long revisionStart) throws SQLException {
    	...
    	for (int i = start; i < revisionList.size(); i++) {
    		...
    		ps.executeBatch();
    		double timeB=(double)(System.currentTimeMillis()-timeA)/1000;
		...
		Iterator iterator = entries.iterator();
		  ...
		  while (iterator.hasNext()) {
	            ...
	            if (entry.getKind() == SVNNodeKind.DIR) {
	                listEntries(revision,(path2.equals("")) ? entry.getName(): path2 + "/" + entry.getName());
					...
					ps.setString(3,"/" + (path2.equals("") ? "" : path2 + "/")+ entry.getName());//文件完整路径
